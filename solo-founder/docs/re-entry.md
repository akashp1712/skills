...
Every day you work on something, you built a mental model: which branch, what is half-done, what you already tried, what you decided and why. When the day ends, that model evaporates. The code persists. The model does not.

So the start of tomorrow gets spent rebuilding it — unless you close today on purpose. Overnight is enough for the model to fade. A weekend without a landing is a Monday that starts at the dashboard.

**The fix is not better memory. It is refusing to end the day without writing the handoff.**
