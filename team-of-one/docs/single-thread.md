...
This is the most uncomfortable Amazon principle for a solo builder, because on its face it disqualifies the entire enterprise. You are, definitionally, making everything somebody's part-time job.
...
A thread is one outcome held open. Single-threading means **exactly one thread is open at any moment** — within a session, and within a week.
...
Two threads do not run at 50% each. They run at roughly 35%, and the missing 30% is reload.
